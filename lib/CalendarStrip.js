...
class CalendarStrip extends Component {
  ...
  getInitialDates(weekStartsOn=0) {
    ...
    const initialDate = this.props.initialDate || new Date();
    const _year = initialDate.getFullYear();
    const _month = initialDate.getMonth();
    const _day = initialDate.getDate();
    const d = new Date(_year, _month, _day); // FORMAT: Wed May 16 2018 00:00:00 GMT+0800 (CST)

    const last2Week = subDays(d, 7 * 2);
    const next2Week = addDays(d, 7 * 2);
    const startLast2Week = startOfWeek(last2Week, { weekStartsOn });
    const endNext2Week = endOfWeek(next2Week, { weekStartsOn });
  ...
  momentumEnd = (event) => {
    ...
    */
    ...
    const daysBeforeToday = differenceInDays(firstDayInCalendar, this.props.initialDate || new Date());
